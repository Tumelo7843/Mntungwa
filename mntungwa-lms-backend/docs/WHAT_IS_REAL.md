...
- [ ] Verify the migration-022 `learner_submissions_delete_orphan` policy on
      hosted Supabase: as an enrolled learner, upload an object to your own
      `{enrollment_id}/…` path and confirm you *can* delete it while it has no
      `submission_files` row, and *cannot* once `finalize_submission_version()`
      has attached it. As a second learner, confirm you cannot delete an object
      in the first learner's folder.
